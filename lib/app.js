...
async.parallel([
        ...
        app.use(expressJwt({
            secret: (req, payload, cb) => {
                return Jwts.findOne({
                    ...
                    let userKeyRaw = cryptoJS.AES.decrypt(payload.userKeyServerEncrypted, new Buffer(row.password + process.env.SERVER_SECRET).toString('base64'));
                    payload.userKey = cryptoJS.enc.Utf8.stringify(userKeyRaw);
    ...
    (cb) => { //load the routes
        return recursive(path.join(__dirname, "endpoint"), [".js"], (err,files) => {
            if (err) {
                return cb(err);
            }
            let filenames = [];
